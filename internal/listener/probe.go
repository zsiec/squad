...
// or fall back to the `tick`-based polling path. Sandboxed environments
// (some CI runners, container security policies) deny loopback bind;
// squad must not silently install a hook that will fail every session
// on those hosts.
