...
func TestInstall_CopiesAllFiles(t *testing.T) {
	src := fstest.MapFS{
		"plugin.json":          {Data: []byte(`{"name":"squad"}`)},
	...
	for _, want := range []string{"plugin.json", "skills/squad-loop.md", "commands/work.md"} {
...
func TestInstall_AtomicReplace(t *testing.T) {
	src := fstest.MapFS{"plugin.json": {Data: []byte(`{"name":"squad","version":"new"}`)}}
	dst := t.TempDir()
	...
	if err := os.WriteFile(filepath.Join(pluginDir, "plugin.json"), []byte(`{"name":"squad","version":"old"}`), 0o644); err != nil {
