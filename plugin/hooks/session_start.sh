...
AGENT_ID="agent-$(openssl rand -hex 2 2>/dev/null || printf '%s' "$$" | cut -c1-4)"
AGENT_NAME="claude-$(date +%s 2>/dev/null || printf 'unknown')"
...
NEXT_TOP=$("$SQUAD_BIN" next --limit 3 --json 2>/dev/null \
