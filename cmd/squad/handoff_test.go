...
func TestRunHandoff_RejectsEmpty(t *testing.T) {
	...
	if code := runHandoffBody(context.Background(), f.chat, st, f.db, f.repoID, "", f.agentID, chat.HandoffBody{}, handoffOpts{}); code == 0 {
...
func TestRunHandoff_StoresAndReleasesClaim(t *testing.T) {
	...
	if code := runHandoffBody(context.Background(), f.chat, st, f.db, f.repoID, "", f.agentID, h, handoffOpts{}); code != 0 {
